...
xferBenchNixlWorker::allocateMemory(int num_threads) {
    ...
    if (xferBenchConfig::isObjStorageBackend()) {
        ...
        // (network-bound), so seed all objects concurrently instead of
        // one-at-a-time; the descriptor/registration bookkeeping below stays
        // sequential. Errors are recorded in a flag rather than exit()-ing inside
        // the OpenMP region.
...
xferBenchNixlWorker::deallocateMemory(std::vector<std::vector<xferBenchIOV>> &iov_lists) {
    ...
    if (xferBenchConfig::isObjStorageBackend()) {
        // Deregister (must stay ordered: remote registrations may reference local
        // buffers) and collect the object keys, then remove the objects
        // concurrently — each rmObj is one HTTP DELETE (network-bound).
        std::vector<std::string> obj_names;
