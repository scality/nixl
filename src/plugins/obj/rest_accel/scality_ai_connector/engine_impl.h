...
private:
    ...
    /// RDMA token client (DC via cuObjClient, or RC via libibverbs)
    std::shared_ptr<iRdmaTokenClient> cuClient_;
    /// True when using DC transport
    bool useDcTransport_ = false;
    /// Scality AI Connector HTTP client with RDMA support
    std::shared_ptr<iScalityRdmaClient> connectorClient_;
};
