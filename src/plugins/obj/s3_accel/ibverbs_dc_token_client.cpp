...
#include "ibverbs_dc_token_client.h"
...
int
S3IbverbsDcTokenClient::numLagPorts(const char *dev_name) {
...
bool
S3IbverbsDcTokenClient::setupNic(const std::string &ip, uint64_t dc_key, NicCtx &nic) {
...
S3IbverbsDcTokenClient::S3IbverbsDcTokenClient(const std::vector<std::string> &nic_ips,
                                                   uint64_t dc_key) {
    if (nic_ips.empty()) {
...
S3IbverbsDcTokenClient::~S3IbverbsDcTokenClient() {
    {
...
bool
S3IbverbsDcTokenClient::isConnected() const {
    return connected_;
}

const std::vector<int> &
S3IbverbsDcTokenClient::affineNicsFor(int dev_id) {
...
int
S3IbverbsDcTokenClient::leastLoadedNic(const std::vector<int> &candidates) {
...
int
S3IbverbsDcTokenClient::selectNicFor(int dev_id) {
...
cuObjErr_t
S3IbverbsDcTokenClient::cuMemObjGetDescriptor(void *ptr, size_t size, int dev_id) {
    if (!connected_) {
        return CU_OBJ_FAIL;
    }
    if (size > UINT32_MAX) {
...
cuObjErr_t
S3IbverbsDcTokenClient::cuMemObjPutDescriptor(void *ptr) {
    std::lock_guard<std::mutex> lk(mu_);
...
void
S3IbverbsDcTokenClient::logAssignment() {
...
std::string
S3IbverbsDcTokenClient::descriptorFor(void *ptr, size_t size) {
...
ssize_t
S3IbverbsDcTokenClient::cuObjGet(void *ctx, void *ptr, size_t size, loff_t, loff_t) {
...
ssize_t
S3IbverbsDcTokenClient::cuObjPut(void *ctx, void *ptr, size_t size, loff_t, loff_t) {
    std::string desc = descriptorFor(ptr, size);
    if (desc.empty()) {
