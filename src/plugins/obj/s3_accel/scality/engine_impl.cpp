...
S3ScalityObjEngineImpl::S3ScalityObjEngineImpl(const nixlBackendInitParams *init_params)
    ...
    s3Client_ = std::make_shared<awsS3ScalityClient>(params_to_use, executor_);
    NIXL_INFO << "Object storage backend initialized with S3 Scality RING client";
}
...
S3ScalityObjEngineImpl::S3ScalityObjEngineImpl(const nixlBackendInitParams *init_params,
    ...
    s3Client_ = s3_client ? s3_client
                          : std::make_shared<awsS3ScalityClient>(params_to_use, executor_);
    NIXL_INFO << "Object storage backend initialized with S3 Scality RING client";
}

iS3Client *
S3ScalityObjEngineImpl::getClient() const {
